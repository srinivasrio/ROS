...
import { ChevronLeft as LucideChevronLeft, FileText as LucideFileText, Check as LucideCheck, Clock as LucideClock, ChefHat as LucideChefHat, Utensils as LucideUtensils, CheckCircle as LucideCheckCircle, Printer as LucidePrinter, Ticket as LucideTicket, X as LucideX } from 'lucide-react';
...
export default function CustomerOrdersPage() {
    ...
    const handleApplyCoupon = async () => {
        ...
        try {
            ...
            const activeOrder = await OrderService.getActiveOrderForTable(tableNumber, restaurantId || urlRestaurantId);
    ...
    useEffect(() => {
        const checkTable = async () => {
            if (tableNumber) {
                const exists: any = await OrderService.verifyTableExists(urlRestaurantId, tableNumber);
                if (exists) {
                    if (exists.restaurant_id !== urlRestaurantId) {
                        setIsValidTable(false);
                        return;
                    }
                    setIsValidTable(true);
                    setRestaurantId(exists.restaurant_id);
                } else {
                    setIsValidTable(false);
                }
            }
    ...
    useEffect(() => {
        ...
        const sub = OrderService.subscribeToOrders(restaurantId, (payload) => {
            if (payload.new && (payload.new as any).table_id === tableNumber) {
                fetchOrder();
            }
        });
    ...
    const confirmBillRequest = async () => {
        try {
            if (!restaurantId) throw new Error("Restaurant ID not loaded");
    ...
    if (!order) {
        return (
            <div className="min-h-screen bg-gray-50 flex flex-col justify-center items-center p-8 text-center">
                ...
                <p className="text-black mb-8 font-medium">This table has been cleared.<br />Thank you for dining with us!</p>
                <button
