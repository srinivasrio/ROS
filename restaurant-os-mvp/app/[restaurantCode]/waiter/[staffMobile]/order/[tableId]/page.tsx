...
import { IndianRupee as LucideIndianRupee } from 'lucide-react';
...
export default function OrderDetails() {
    ...
    const loadOrder = async () => {
        ...
        try {
            const data = await OrderService.getActiveOrderForTable(tableId as string | number, restaurantId);
            if (activeRef.current) setOrder(data);
        } catch (err) {
    ...
    useEffect(() => {
        ...
        if (!restaurantLoading && restaurantId) {
            ...
            const subscription = OrderService.subscribeToOrders(restaurantId, () => {
                loadOrder();
            });

            const serviceSubscription = OrderService.subscribeToServiceRequests(restaurantId, payload => {
                if (payload.eventType === 'INSERT' || payload.eventType === 'DELETE' || payload.eventType === 'UPDATE') {
                    const newRecord = payload.new as any;
                    const oldRecord = payload.old as any;
                    const isRelevant = order?.associated_table_ids 
                        ? (order.associated_table_ids.includes(Number(newRecord?.table_id)) || order.associated_table_ids.includes(Number(oldRecord?.table_id)))
                        : (String(newRecord?.table_id) === String(tableId) || String(oldRecord?.table_id) === String(tableId));
        ...
        try {
            if (action === 'accept') {
                ...
                setTimeout(async () => {
                    if (!restaurantId) return;
                    try {
                        await OrderService.completeServiceRequest(requestId, restaurantId);
    ...
    const handleServeItem = async (itemId: string) => {
        if (!restaurantId) return;
        try {
            await OrderService.updateOrderItemStatus(itemId, restaurantId, 'served');
            const freshOrder = await OrderService.getActiveOrderForTable(tableId as string | number, restaurantId);
            if (freshOrder) {
                const allServed = freshOrder.items && freshOrder.items.every((i: any) => i.status === 'served');
                if (allServed && freshOrder.status !== 'served') {
                    await OrderService.updateOrderStatus(freshOrder.id, restaurantId, 'served');
    ...
    const confirmSettle = async () => {
        if (!order || !restaurantId) return;
        try {
            await OrderService.settleBill(order.id, restaurantId, undefined, 'Waiter');
    ...
    const confirmClear = async () => {
        if (!tableId || !restaurantId) return;
        try {
            await OrderService.clearTable(tableId as string | number, restaurantId);
        ...
        <div className="flex flex-col h-full bg-white relative">
            ...
            <header className="flex items-center bg-white p-4 pb-2 justify-between sticky top-0 z-10 border-b border-divider shrink-0">
                <div className="flex items-center gap-3">
                    <Link href={`/${restaurantId}/waiter/${staffMobile}/dashboard`} className="text-charcoal active:opacity-50">
                ...
                <h2 className="text-charcoal text-lg font-bold leading-tight tracking-tight flex-1 text-center">
                    {order?.table_number ? `Table ${order.table_number}` : isNaN(Number(tableId)) ? 'Merged Group' : `Table ${tableId}`}
                </h2>
            ...
            <main className="flex-1 overflow-y-auto pb-32 bg-light-gray/30 p-4">
                                ...
                                return (
                                    <div key={req.id} className={`border rounded-xl p-3 flex items-center justify-between shadow-sm animate-in slide-in-from-top-2 ${details.bg} ${details.borderColor}`}>
                                        ...
                                        {req.status === 'delivered' && (
                                            <span className="px-4 py-2 bg-gray-100 text-black text-xs font-bold uppercase tracking-wider rounded-lg border border-gray-200">
                                                Delivered
                                            </span>
                                        )}
                                    </div>
                ...
                {!isOccupied ? (
                    <div className="flex flex-col items-center justify-center h-full space-y-6 text-center">
                        <div className="size-20 bg-gray-100 rounded-full flex items-center justify-center text-black">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-black mt-1">Scan QR Code to start a new order.</p>
                        </div>
                        <Link
                            href={`/${restaurantId}/waiter/${staffMobile}/menu/${tableId}`}
                        ...
                        {isNaN(Number(tableId)) && (
                            <button
                                onClick={async () => {
                                    if (!restaurantId) return;
                                    try {
                                        await OrderService.unmergeTables(tableId as string, restaurantId);
                                        router.push(`/${restaurantId}/waiter/${staffMobile}/dashboard`);
                                    } catch (err) {
                                        console.error('Failed to unmerge tables:', err);
                                        alert('Failed to unmerge tables');
                                    }
                                }}
                                className="w-full py-4 mt-2 bg-white border border-neutral-200 text-black font-bold rounded-xl shadow-sm active:bg-neutral-50 transition-colors flex items-center justify-center gap-2"
                            >
                    ...
                    <>
                        ...
                        <div className={`bg-white rounded-xl border ${isPaid ? 'border-green-200 ring-2 ring-green-100' : 'border-divider'} shadow-sm overflow-hidden mb-4 transition-all duration-300`}>
                            <div className={`px-4 py-3 border-b border-divider flex justify-between items-center ${isPaid ? 'bg-green-50' : 'bg-gray-50'}`}>
                                <span className={`text-xs font-bold tracking-widest uppercase ${isPaid ? 'text-green-700' : 'text-black'}`}>
                            ...
                            <div className="divide-y divide-dashed divide-divider">
                                {order.items?.map((item: any, idx) => (
                                    <OrderItem
                                        ...
                                        isServed={item.status === 'served' || item.status === 'paid'}
                                        price={item.price}
                                        notes={item.notes}
                        ...
                        {isPaid ? (
                            ...
                            <>
                                <p className="text-black text-[11px] font-bold leading-normal pb-3 pt-2 uppercase tracking-[0.15em]">Actions</p>
                                <div className="grid grid-cols-2 gap-3">
                                    <Link href={`/${restaurantId}/waiter/${staffMobile}/menu/${tableId}`} className="flex flex-col items-center justify-center p-4 bg-white border border-divider rounded-xl shadow-sm active:bg-gray-50 transition-colors">
                                    ...
                                    <button
                                        ...
                                        disabled={
                                            ...
                                            !order.items?.some((i: any) => i.status === 'ready') ||
                                            order.items?.some((i: any) => i.status === 'preparing' || i.status === 'placed')
                                        }
                                        className={`flex flex-col items-center justify-center p-4 bg-white border border-divider shadow-sm transition-colors rounded-xl ${
                                            (order.status === 'served' || order.status === 'paid' || order.status === 'cancelled' || !order.items?.some((i: any) => i.status === 'ready') || order.items?.some((i: any) => i.status === 'preparing' || i.status === 'placed'))
                                    ...
                                    >
                                        ...
                                        <span className="text-xs font-bold text-charcoal uppercase tracking-wide">
                                            {order.status === 'served' ? 'Served' : 'Mark Served'}
                                        </span>
                                    </button>
                                    <button
                                        disabled={true}
                                    ...
                                    >
                                        <span className="material-icons-outlined text-black text-3xl mb-2">print</span>
                                        <span className="text-xs font-bold text-black uppercase tracking-wide">Print KOT (Pay First)</span>
                                    </button>
                                    <button
                    ...
                    <div className="w-[85%] max-w-sm bg-white rounded-2xl shadow-2xl p-6 flex flex-col items-center text-center animate-in zoom-in-95 duration-200 scale-100">
                        ...
                        <div className="flex gap-3 w-full">
                            <button
                                ...
                                className="flex-1 py-3 px-4 bg-gray-100 text-black font-bold rounded-xl active:scale-95 transition-transform"
                            >
                                Cancel
                            </button>
                            <button
...
function OrderItem({ id, name, qty, status, isReady, isServed, price, notes, onServe, isReadOnly, createdAt, estimatedEnd, servedAt, imageUrl, itemType, comboItems }: { id: string, name: string; qty: number; status: string; isReady?: boolean; isServed?: boolean; price: number; notes?: string, onServe: () => void, isReadOnly?: boolean, createdAt: string, estimatedEnd?: string, servedAt?: string, imageUrl?: string, itemType?: string, comboItems?: any[] }) {
    if (itemType?.toLowerCase() === 'combo') {
        return (
            <div className={`p-4 flex flex-col gap-2 ${isReady ? 'bg-green-50/50' : ''}`}>
                <SharedComboCard
                    name={name}
                    image_url={imageUrl}
                    price={price}
                ...
                />
                        <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                {isServed && servedAt ? (
                                    ...
                                    <Timer
                                        startTime={createdAt}
                                        className="text-[10px] font-medium text-black"
                                        prefix=""
                                    />
                                )}
                            </div>

                            {status === 'preparing' && estimatedEnd && (
                                <div className="flex items-center gap-2">
                                    <span className="text-[10px] text-black">•</span>
                                    <CountdownTimer 
                                        status={status} 
                    ...
                    <div className="text-right shrink-0">
                        {!isServed && status === 'ready' && !isReadOnly && (
                            <button
                                onClick={onServe}
                                className="flex items-center gap-1 px-3 py-1.5 bg-green-600 text-white text-[10px] font-black uppercase tracking-wider rounded-lg shadow-sm shadow-green-200 active:scale-95 transition-transform"
                            >
                                <span>SERVE</span>
                            </button>
                        )}
                        {isServed && (
                            <span className="text-[10px] font-bold text-black uppercase tracking-wider block">SERVED</span>
                        )}
                    </div>
                </div>
        ...
        <div className={`p-4 flex gap-3 ${isReady ? 'bg-green-50/50' : ''}`}>
            <div className="relative size-14 rounded-lg overflow-hidden shrink-0 border border-gray-100 bg-gray-50 flex items-center justify-center">
                {imageUrl ? (
                    ...
                    <span className="material-icons-outlined text-black text-2xl">restaurant</span>
                )}
            </div>
                ...
                <div className="flex justify-between items-start">
                    <div className="flex items-start gap-2">
                        ...
                        <div>
                            ...
                            {notes && <p className="text-[10px] text-black italic mt-0.5">{notes}</p>}
                    ...
                    <div className="text-right shrink-0 ml-2">
                        ...
                        {/* Show serve button only if item is marked ready by chef */}
                        {!isServed && status === 'ready' && !isReadOnly && (
                            <button
                                onClick={onServe}
                        ...
                        {isServed && (
                            <span className="text-[10px] font-bold text-black uppercase tracking-wider mt-1 block">SERVED</span>
                        )}
                    </div>
                </div>
            </div>
