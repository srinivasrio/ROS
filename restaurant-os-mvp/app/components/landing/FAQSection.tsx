...
const FAQSection = () => {
  ...
  return (
    <section id="faq" className="py-24 relative bg-background">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-purple-500/5 rounded-full blur-[200px]" />
      <div className="relative z-10 max-w-3xl mx-auto px-6">
        <RevealMotion className="text-center mb-16">
          <span className="gradient-text-hero text-sm font-bold uppercase tracking-widest">Got Questions?</span>
          <h2 className="text-3xl md:text-5xl font-bold mt-3" >Frequently Asked</h2>
        </RevealMotion>
        <div className="space-y-3">
          {faqs.map((f, i) => (
            <RevealMotion key={i} delay={0.05 * i}>
              <motion.div
                layout
                className={`rounded-xl border border-neutral-200 dark:border-neutral-800 overflow-hidden ${openIndex === i ? "bg-neutral-50 dark:bg-neutral-900" : "bg-neutral-50/50 dark:bg-neutral-900/30"}`}
              >
                <button
                  onClick={() => setOpenIndex(openIndex === i ? null : i)}
                  className="w-full flex items-center justify-between p-5 text-left"
                >
                  <span className={`font-bold text-sm md:text-base transition-colors ${openIndex === i ? "text-[#FF6B6B]" : "text-black dark:text-black"}`} >
                    {f.q}
                  </span>
                  <motion.span
                    animate={{ rotate: openIndex === i ? 45 : 0 }}
                    transition={{ duration: 0.3 }}
                    className={`text-xl ml-4 flex-shrink-0 ${openIndex === i ? "text-[#FF6B6B]" : "text-black"}`}
                  >
                    +
                  </motion.span>
                </button>
                ...
                >
                  <p className="px-5 pb-5 text-sm text-black dark:text-black leading-relaxed font-medium">{f.a}</p>
                </motion.div>
              </motion.div>
            </RevealMotion>
          ))}
