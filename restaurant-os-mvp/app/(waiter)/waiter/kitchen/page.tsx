...
export default function WaiterKitchen() {
    ...
    useEffect(() => {
        ...
        const sub = OrderService.subscribeToOrders(() => loadData());
        const subItems = OrderService.subscribeToOrderItems(() => loadData());
    ...
    const handleOrderClick = (tableId: number | string | null | undefined, mergeGroupId?: string | null) => {
        if (mergeGroupId) {
            router.push(`/waiter/order/${mergeGroupId}`);
        } else if (tableId) {
    ...
    const getTableName = (order: Order) => {
        ...
        return `T${order.table_id}`;
    };

    if (loading) {
        return <div className="flex h-full items-center justify-center text-gray-400">Loading Kitchen View...</div>;
    }
    ...
    return (
        <div className="flex flex-col h-full bg-white">
            ...
            {/* Main List */}
            <main className="flex-1 overflow-y-auto pb-6 bg-light-gray/30">
                                    ...
                                    <div className="p-3 bg-green-50 border-b border-green-100 flex justify-between items-center">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div className="flex flex-col">
                                                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">ORDER #{order.id.slice(0, 4)}</span>
                                    ...
                                    <div className="p-3 flex-1 flex flex-col justify-between">
                                        <div className="mb-3">
                                            {order.items?.filter((i: any) => i.status === 'ready').map((item: any, idx: number) => (
                                                <div key={idx} className="flex justify-between items-start text-sm text-gray-700 font-bold leading-tight mb-1">
                                                    ...
                                                    <div className="flex justify-between items-center mb-1">
                                                        ...
                                                        <span className={`text-[9px] font-black uppercase ${item.status === 'ready' ? 'text-green-500' :
                                                            item.status === 'paid' ? 'text-green-500' :
                                                                item.status === 'preparing' ? 'text-orange-500' : 'text-gray-400'
                                                            }`}>
