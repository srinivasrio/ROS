...
import { LucideChefHat, LucideCheckCircle, LucideUtensils, LucideClock, LucideAlertCircle } from 'lucide-react';
...
export default function KitchenTicket({ order, onStatusChange, onItemStatusChange, isReadOnly = false, density = 'comfortable' }: KitchenTicketProps) {
        ...
        >
            ...
            <div className={`${isCompact ? 'p-2 pl-3' : 'p-3 pl-3'} flex justify-between items-center border-b border-black/5 bg-white/40 backdrop-blur-sm relative`}>
                <div className="flex flex-col">
                    <div className="flex items-center gap-2">
                        <h3 className={`${isCompact ? 'text-[10px]' : 'text-[11px]'} font-black text-neutral-800 tracking-tight uppercase leading-none`}>Table {order.table_id}</h3>
                        {isLate && (
                ...
                <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-neutral-900/[0.03] ring-1 ring-black/5">
                    <span className={`${isCompact ? 'text-[9px]' : 'text-[10px]'} font-mono font-black ${isLate ? 'text-red-600' : 'text-neutral-900/40'} tracking-tighter`}>
                        {elapsedStr}
                    </span>
                </div>
            </div>
            ...
            <div className="flex flex-col bg-transparent">
                {(order.items?.filter(item => {
                    ...
                    <div key={idx} className="flex gap-2.5 items-center px-3 py-2 border-b border-neutral-100/60 last:border-b-0 hover:bg-neutral-50/50 transition-colors">
                        ...
                        <div className="flex-1 min-w-0 flex items-center gap-3">
                            <span className={`shrink-0 flex items-center justify-center ${isCompact ? 'h-4 min-w-[16px] text-[8px]' : 'h-5 min-w-[20px] text-[9px]'} rounded-md bg-neutral-900/5 text-neutral-900/80 font-black shadow-inner`}>
                                {item.quantity}
                            </span>
                            <div className="flex-1 min-w-0">
                                <p className={`${isCompact ? 'text-[10px]' : 'text-[11px]'} font-bold text-neutral-800/90 tracking-tight truncate`}>{item.name}</p>
                                {item.notes && (
                        ...
                        <div className="shrink-0 flex items-center gap-1.5">
                            ...
                            {item.status === 'served' && (
                                <span className="text-[8px] font-black text-neutral-400 bg-neutral-100/50 px-2 py-0.5 rounded-full border border-neutral-200 uppercase tracking-wider">
                                    Done
                                </span>
                            )}
                        </div>
